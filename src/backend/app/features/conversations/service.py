...
def _news_digest_summary_prompt(
    digest: NewsDigest,
    content: str,
    language: UserLanguage,
) -> List[Dict[str, str]]:
...
def _digest_with_summary(digest: NewsDigest, summary: str) -> NewsDigest:
...
def _digest_with_llm_output(digest: NewsDigest, output: str) -> NewsDigest:
    parsed = _extract_json_object(output)
    if parsed is None:
        return _digest_with_summary(digest, output[:4000])
