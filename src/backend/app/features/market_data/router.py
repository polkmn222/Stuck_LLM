...
from app.features.market_data.schemas import MarketChartWindow, MarketQuote
...
def read_quote(
    market: str,
    symbol: str,
    window: MarketChartWindow = Query("1D"),
) -> MarketQuote:
    quote = get_quote(market, symbol, window=window)
    if quote is None:
